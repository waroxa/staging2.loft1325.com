...
function nd_options_customizer_header( $wp_customize ) {
	...
	$wp_customize->add_panel( 'nd_options_customizer_header_panel', array(
	  ...
	  'theme_supports' => '',
	  'description' => __( 'Header Settings','nd-shortcodes' ), //  html tags such as <p>.
...
function nicdark_headers() {
	...
	$nd_options_layout_selected = dirname( __FILE__ ).'/'.$nd_options_customizer_header_layout.'/content.php';
...
function nd_options_customizer_header_style(){
	...
	$nd_options_layout_selected = dirname( __FILE__ ).'/'.$nd_options_customizer_header_layout.'/style.php';
  	include realpath($nd_options_layout_selected);	
