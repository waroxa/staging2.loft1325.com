...
class SettingsMenuPage extends AbstractMenuPage {
	...
	private function _generateGeneralTab() {
		...
		$currencyGroupFields = array(
			...
			Fields\FieldFactory::create(
				'mphb_decimals_separator',
				array(
					'type'    => 'text',
					'label'   => esc_html__( 'Decimal Separator', 'motopress-hotel-booking' ),
					'default' => '.',
					'size'    => 'small',
				)
			),
			Fields\FieldFactory::create(
				'mphb_thousand_separator',
				array(
					'type'        => 'thousand-separator',
					'label'       => esc_html__( 'Thousand Separator', 'motopress-hotel-booking' ),
					'default'     => ',',
					'size'        => 'small',
					'allow_space' => true,
				)
			...
			Fields\FieldFactory::create(
				...
				array(
					'type'    => 'number',
					'label'   => esc_html__( 'Number of Decimals', 'motopress-hotel-booking' ),
					'min'     => 0,
					'step'    => 1,
					'default' => 2,
				)
		...
		$searchParametersFields = array(
			...
			Fields\FieldFactory::create(
				'mphb_use_occupancy_presets_on_checkout',
				array(
					'type'        => 'checkbox',
					'default'     => false,
					'inner_label' => esc_html__( "Remember the user's selected number of guests until the checkout page.", 'motopress-hotel-booking' ),
				)
			),
			Fields\FieldFactory::create(
		...
		$iCalFields = array(
			...
			Fields\FieldFactory::create(
				'mphb_ical_sync_with_buffers',
				array(
					'type'        => 'checkbox',
					'default'     => false,
					'inner_label' => esc_html__( 'Export and import bookings with buffer time included.', 'motopress-hotel-booking' ),
				)
			),
			Fields\FieldFactory::create(
