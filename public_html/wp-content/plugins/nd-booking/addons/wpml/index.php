...
function nd_booking_add_wpml_field(){
	...
	$nd_booking_add_wpml_field = '<div class="nd_booking_section nd_booking_height_20"></div>';

	$nd_booking_add_wpml_field .= '
	
	<label class="nd_booking_section">
		<input '; if( get_option('nd_booking_integration_wpml') == 1 ) { $nd_booking_add_wpml_field .= 'checked="checked"'; } $nd_booking_add_wpml_field .= ' name="nd_booking_integration_wpml" type="checkbox" value="1">';
		$nd_booking_add_wpml_field .= __('WPML Integration','nd-booking');
	...
	echo wp_kses( $nd_booking_add_wpml_field, $nd_booking_allowed_html );
}
...
if ( $nd_booking_integration_wpml == 1 and get_option('nicdark_theme_author') == 1 ) {
	...
	function nd_booking_add_id_metabox_on_cpt_1(){
		...
		echo wp_kses( $nd_booking_tab_id, $nd_booking_allowed_html );
	...
	function nd_booking_add_id_metabox_content_on_cpt_1(){
		...
		echo wp_kses( $nd_booking_tab_ids_content, $nd_booking_allowed_html );
