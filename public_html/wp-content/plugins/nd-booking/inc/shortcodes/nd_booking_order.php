...
function nd_booking_shortcode_order() {
    ...
    if ( is_user_logged_in() == 1 ){
        ...
        if ( $nd_booking_current_user_id == $nd_booking_order_user_id ) {
            ...
            $nd_booking_order_infos_query = $wpdb->prepare( "SELECT * FROM $nd_booking_table_name WHERE id = %d", $nd_booking_order_id );
            $nd_booking_order_infos = $wpdb->get_results( $nd_booking_order_infos_query );
            ...
            foreach ( $nd_booking_order_infos as $nd_booking_order_info ) 
            {
                ...
                }elseif ( $nd_booking_order_action_type == 'paypal' ){
                    $nd_booking_order_action_type_lang = __('Paypal','nd-booking');   
                }else{
                    $nd_booking_order_action_type_lang = $nd_booking_order_action_type; 
                }
                ...
                include realpath(dirname( __FILE__ ).'/include/order/nd_booking_order_left_content.php'); 
                include realpath(dirname( __FILE__ ).'/include/order/nd_booking_order_right_content.php'); 
    ...
    return $nd_booking_shortcode_order;
