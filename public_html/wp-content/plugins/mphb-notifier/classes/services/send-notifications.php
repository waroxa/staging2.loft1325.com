...
class SendNotifications extends BackgroundProcess {
	...
	protected function triggerNotification( $notificationId ) {
		...
		$notification = mphb_notifier_get_notification( $notificationId );
	...
	protected function triggerNotificationOnBooking( $booking, $notification ) {
		...
		switch ( $notification->type ) {
			...
			case 'email':
				add_filter(
					'_mphb_notifier_booking_language',
					function () use ( $booking ) {
						return $booking->getLanguage();
					}
				);

				$originalId = apply_filters( 'wpml_object_id', absint( $notification->id ), 'mphb_notification', true, $booking->getLanguage() );

				if ( absint( $originalId ) != absint( $notification->id ) ) {
					$notification = mphb_notifier_get_notification( $originalId );
				}

				return mphb_notifier_trigger_emails( $booking, $notification );
				break;
