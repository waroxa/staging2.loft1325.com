...
class Plugin {
	...
	/** @var \MPHB\Notifier\Containers\EmailsContainer */
	private $emailsContainer = null;
	...
	public function load() {
		...
		$this->emailsContainer       = new Containers\EmailsContainer();
		...
		add_action(
			...
			function() {
				...
				if ( ! wp_doing_ajax() ) {
					...
					new Admin\MetaBoxes\NoticesMetaBox( 'mphb_notification_notices', esc_html__( 'Notification Notices', 'mphb-notifier' ), mphb()->postTypes()->roomType()->getPostType(), 'normal' );
					// translators: "Test" as a verb
					new Admin\MetaBoxes\TestMetaBox( 'mphb_notification_test', esc_html__( 'Test Notification Now', 'mphb-notifier' ), mphb_notifier_get_notification_post_type(), 'side' );
				}
			}
		);
	...
	/**
	 * @return \MPHB\Notifier\Containers\EmailsContainer
	 */
	public function emails() {
		return $this->emailsContainer;
	}
