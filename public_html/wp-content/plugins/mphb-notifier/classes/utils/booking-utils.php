...
class BookingUtils {
	...
	public static function isNotificationFitsToGoForNewBooking( $notification, $booking ) {
		...
		if ( $notification->trigger['compare'] == 'after' ||
			$notification->trigger['field'] == 'check-out' ) {

			return false;
		}

		return static::isNotificationFitsToGoForBookingReservedRooms( $notification, $booking ) &&
			$daysToCheckIn <= $notification->trigger['period'];
	}
	...
	public static function isNotificationDisabledForReservationAfterTrigger( $notification, $booking ) {
		...
		if ( $notification->is_disabled_for_reservation_after_trigger && 'before' == $notification->trigger['compare'] ) {

			$daysToTrigger = 0;

			if ( 'check-in' == $notification->trigger['field'] ) {

				$daysToTrigger = \MPHB\Utils\DateUtils::calcNights( $booking->getDateTime(), $booking->getCheckInDate() );

			} elseif ( 'check-out' == $notification->trigger['field'] ) {

				$daysToTrigger = \MPHB\Utils\DateUtils::calcNights( $booking->getDateTime(), $booking->getCheckOutDate() );
			}

			$result = $daysToTrigger < $notification->trigger['period'];
		}
	...
	public static function isNotificationFitsToGoForBookingReservedRooms( $notification, $booking ) {
		...
		foreach ( $reservedRooms as $room ) {
			...
			if ( ! is_array( $notification->accommodationTypeIds ) ||
				empty( $notification->accommodationTypeIds ) ||
				in_array( 0, $notification->accommodationTypeIds ) || // All accommodation types
				in_array( $room->getRoomTypeId(), $notification->accommodationTypeIds )
			) {
