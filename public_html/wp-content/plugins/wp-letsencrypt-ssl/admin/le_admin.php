...
class WPLE_Admin {
    ...
    public function wple_menu_page() {
        if ( FALSE === get_option( 'wple_version' ) ) {
            ...
            if ( version_compare( get_option( 'wple_version' ), '7.8.5.5', '<=' ) ) {
                delete_option( 'wple_plan_choose' );
                update_option( 'wple_version', WPLE_PLUGIN_VER );
            }
        }
    ...
    public function wple_upgrade_block( &$html ) {
      ...
      <div id="wple-upgradepro">';
        if ( FALSE !== $cp && $cp ) {
            $html .= '<strong style="display: block; text-align: center; color: #666;">' . __( "Woot Woot! You have <b>CPANEL</b>! Why struggle with manual SSL renewal every 90 days? - Enjoy 100% automation with PRO version.", 'wp-letsencrypt-ssl' ) . '</strong>';
