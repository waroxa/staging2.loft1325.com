...
/**
 * Determine if the current request is the ND Booking room selection page.
 *
 * @return bool
 */
function marina_child_is_booking_flow_room_selection() {
    if ( is_admin() ) {
        return false;
    }
...
/**
 * Get the current language code for mobile header toggles.
 *
 * @return string
 */
function marina_child_get_language_code() {
    if ( function_exists( 'trp_get_current_language' ) ) {
    ...
    $locale = get_locale();
    return ( strpos( $locale, 'en_' ) === 0 ) ? 'en' : 'fr';
}

/**
 * Render the mobile header for the booking flow page.
 */
function marina_child_render_mobile_booking_header() {
    if ( ! wp_is_mobile() || ! marina_child_is_booking_flow_room_selection() ) {
        return;
    }

    $language = marina_child_get_language_code();
...
add_action( 'wp_body_open', 'marina_child_render_mobile_booking_header' );

/**
 * Enqueue mobile header behavior for ND Booking pages.
 */
function marina_child_enqueue_mobile_booking_header_assets() {
    if ( ! wp_is_mobile() || ! marina_child_is_booking_flow_room_selection() ) {
        return;
    }

    $script_path = get_stylesheet_directory() . '/js/mobile-booking-header.js';
    $script_uri  = get_stylesheet_directory_uri() . '/js/mobile-booking-header.js';
    $version     = file_exists( $script_path ) ? (string) filemtime( $script_path ) : '1.0.0';

    wp_enqueue_script( 'marina-child-mobile-booking-header', $script_uri, array(), $version, true );
}
add_action( 'wp_enqueue_scripts', 'marina_child_enqueue_mobile_booking_header_assets' );
